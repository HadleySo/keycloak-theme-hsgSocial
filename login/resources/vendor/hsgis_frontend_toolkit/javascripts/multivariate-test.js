;(function (global) {
  ...
  MultivariateTest.prototype.getCohort = function () {
    var cohort = HSGIS.cookie(this.cookieName())
    if (!cohort || !this.cohorts[cohort]) {
      cohort = this.chooseRandomCohort()
      HSGIS.cookie(this.cookieName(), cohort, {days: this.cookieDuration})
    }
    return cohort
  }
  ...
  MultivariateTest.prototype.createDummyEvent = function (cohort) {
    ...
    HSGIS.analytics.trackEvent(this.cookieName(), 'run', {nonInteraction: true})
  }
  ...
  HSGIS.MultivariateTest = MultivariateTest
